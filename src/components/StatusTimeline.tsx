 ...
 * Usa React Query → cache condivisa, nessun re-fetch se il componente si rimonta.
 */
import { useStatusHistory } from '@/hooks/useDashboardQueries';
...
export function StatusTimeline({ measurementId }: Props) {
  const { data: history = [], isLoading } = useStatusHistory(measurementId);
