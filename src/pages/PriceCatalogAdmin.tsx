...
export default function PriceCatalogAdmin() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setCatalog((c.data || []) as PriceCatalogRow[]);
      setModifiers((m.data || []) as PriceModifierRow[]);
      setLoading(false);
    });
  }, [isAdmin]);
  ...
  const saveCatalog = async () => {
    ...
    if (row.id) {
      ...
      const { data, error } = await supabase.from('price_catalog').insert({
        door_model_id: row.door_model_id, width_min_mm: row.width_min_mm, width_max_mm: row.width_max_mm,
        height_min_mm: row.height_min_mm, height_max_mm: row.height_max_mm, base_price: row.base_price,
      }).select('*').single();
      if (error) { toast.error(error.message); setSaving(false); return; }
  ...
  const saveModifier = async () => {
    ...
    if (row.id) {
      const { error } = await supabase.from('price_modifiers').update(payload).eq('id', row.id);
      if (error) { toast.error(error.message); setSaving(false); return; }
      setModifiers(prev => prev.map(r => r.id === row.id ? { ...r, ...payload, id: row.id } as PriceModifierRow : r));
      toast.success('Aggiornato');
    } else {
      const { data, error } = await supabase.from('price_modifiers').insert(payload).select('*').single();
      if (error) { toast.error(error.message); setSaving(false); return; }
